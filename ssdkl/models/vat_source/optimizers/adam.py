...
class ADAM(Optimizer):
    def __init__(self,cost,params,alpha=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-8):
        self.alpha = theano.shared(numpy.array(alpha).astype(theano.config.floatX))
        self.beta_1 = numpy.array(beta_1).astype(theano.config.floatX)
        self.beta_2 = numpy.array(beta_2).astype(theano.config.floatX)
        self.epsilon = numpy.array(epsilon).astype(theano.config.floatX)
